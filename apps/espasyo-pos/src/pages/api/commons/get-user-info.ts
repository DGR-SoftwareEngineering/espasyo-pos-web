  ...
  (client) => async (req, res) => {
    try {
      ...
      try {
        ...
        const axiosError = backendError as AxiosError;
        const status = axiosError?.response?.status;

        // If backend returns 403/401 (e.g., cashier role not authorized), fall back to JWT identity.
        // The auth hook already handles this gracefully; we're just removing the error from the network tab.
