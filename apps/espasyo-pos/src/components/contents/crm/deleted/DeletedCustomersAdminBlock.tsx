...
export const DeletedCustomersAdminBlock: React.FC = () => {
  ...
  const permanentDeleteCb = useApiCallback(async (api, id: string) => api.crm.permanentDelete(id));
  ...
  const handlePermanentDelete = useCallback(async () => {
    ...
    setActionLoading(true);
    try {
      const result = await permanentDeleteCb.execute(deleteTarget.customerID);
      if (result?.data?.success) {
        showToast(`${deleteTarget.fullName} permanently deleted`, "success");
  ...
  }, [deleteTarget, permanentDeleteCb, showToast]);
    ...
    <Box style={{ width: "100%" }}>
      <Card variant="surface" size="3" mb="4">
        <HeaderV2
          ...
          icon={<DeleteForeverOutlined style={{ fontSize: 24, color: "var(--red-11)" }} />}
          extraContent={
            <Flex gap="2">
              <Button variant="soft" color="gray" onClick={() => router.push("/admin/hub/crm/customers")}>
                <ArrowLeftIcon />
                Back to CRM
              </Button>
      ...
      <Card variant="surface" size="3">
        ...
        {loading ? (
          ...
          <Flex align="center" justify="center" py="8" direction="column" gap="2">
            <Text color="gray" size="3">No deleted customers found.</Text>
          </Flex>
        ) : (
                ...
                <Table.Row key={c.customerID}>
                  ...
                  <Table.Cell>
                    <Flex gap="2">
                      ...
                      >
                        <DeleteForeverOutlined style={{ fontSize: 14 }} />
                        Delete Forever
                      </Button>
                    </Flex>
                  </Table.Cell>
      ...
      <AlertDialog.Root open={!!restoreTarget} onOpenChange={(o) => !o && setRestoreTarget(null)}>
        <AlertDialog.Content>
          ...
          <AlertDialog.Description>
            Restore <strong>{restoreTarget?.fullName}</strong>? They will reappear in the active CRM list.
          </AlertDialog.Description>
          <Flex gap="3" mt="4" justify="end">
            ...
            <AlertDialog.Action>
              <Button color="teal" onClick={handleRestore} loading={actionLoading}>
                Restore
              </Button>
            </AlertDialog.Action>
          </Flex>
      ...
      <AlertDialog.Root open={!!deleteTarget} onOpenChange={(o) => !o && setDeleteTarget(null)}>
        <AlertDialog.Content>
          ...
          <AlertDialog.Description>
            This will <strong>permanently</strong> delete <strong>{deleteTarget?.fullName}</strong>.
            This action <strong>cannot be undone</strong>. All associated data will be lost.
          </AlertDialog.Description>
          <Flex gap="3" mt="4" justify="end">
            <AlertDialog.Cancel>
              <Button variant="soft" color="gray">Cancel</Button>
            ...
            <AlertDialog.Action>
              <Button color="red" onClick={handlePermanentDelete} loading={actionLoading}>
                Delete Forever
              </Button>
            </AlertDialog.Action>
          </Flex>
