...
interface VariantRecipeDialogProps {
  variantId: string;
  variantName: string;
  open: boolean;
  onClose: () => void;
...
export const VariantRecipeDialog: React.FC<VariantRecipeDialogProps> = ({
  variantId,
  variantName,
  open,
  onClose,
  ...
  const loadRecipeCb = useApiCallback(
    async (api, id: string) => api.commons.getVariantRecipeByVariant(id),
  );
  const createCb = useApiCallback(
    async (api, params: CreateVariantRecipeParams) =>
      api.commons.createVariantRecipe(params),
  );
  const updateCb = useApiCallback(
    async (api, params: UpdateVariantRecipeParams) =>
      api.commons.updateVariantRecipe(params),
  );
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setDataLoading(true);
      try {
        const result = await loadRecipeCb.execute(variantId);
        if (result.data.success && result.data.response) {
          const recipe = result.data.response;
          setExistingRecipeId(recipe.variantRecipeID);
          setNotes(recipe.notes ?? "");
          setItems(
            [...recipe.recipeItems]
              .sort(
                (a: VariantRecipeItemResponse, b: VariantRecipeItemResponse) =>
                  a.displayOrder - b.displayOrder,
              )
              .map((item: VariantRecipeItemResponse) => ({
    ...
    try {
      ...
      if (existingRecipeId) {
        const result = await updateCb.execute({
          variantRecipeID: existingRecipeId,
          notes: notes || null,
          recipeItems,
        });
        if (result.data.success) {
          showToast("Variant recipe updated", "success");
          onSaved?.();
          onClose();
        } else {
        ...
        if (result.data.success) {
          showToast("Variant recipe created", "success");
          onSaved?.();
          onClose();
        } else {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (result.data.success) {
        showToast("Variant recipe deleted", "success");
        onSaved?.();
        onClose();
      } else {
  ...
  return (
    <DialogBox
      ...
      title={
        isEditMode
          ? `Edit Recipe: ${variantName}`
          : `Add Recipe: ${variantName}`
      }
      subtitle="Override ingredient consumption for this specific variant"
      maxWidth="md"
      loading={isLoading}
      actions={
      ...
      <Flex direction="column" gap="4">
        ...
        <Box>
          ...
          <TextArea
            ...
            placeholder="Add notes for this variant recipe..."
            rows={3}
          />
        </Box>
        ...
        <Callout.Root color="indigo" variant="soft">
          ...
          <Callout.Text>
            <strong>Override recipe:</strong> When this variant is sold, these
            ingredients are deducted instead of the base product recipe. If no
            variant recipe exists, the base product recipe applies.
          </Callout.Text>
        </Callout.Root>
      </Flex>
    </DialogBox>
