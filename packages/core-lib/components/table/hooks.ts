...
import { PaginatedTableParams, SortByValue } from "./types";
    ...
    updateFilters: (filterValue: FilterValue) =>
      setFilters((currFilters) => {
        ...
        if (filterValue.value) {
          return currFilters.map((filter: FilterValue, index: number) =>
            index === currentFilterIndex ? filterValue : filter
          );
        }

        return currFilters.filter(
          (_: FilterValue, index: number) => index !== currentFilterIndex
        );
      }),
    tableProps: {
      filters,
