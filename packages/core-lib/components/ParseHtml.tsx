...
parser.addRule(MATCH_HTML_HEADINGS, (text: string) => ({
  type: "text",
  text: text.replace(MATCH_HTML_HEADINGS, "<h2>$1</h2>"),
}));

parser.addRule(MATCH_LI_WITH_DOUBLE_BRACKETS, ""); // remove li elements which values not injected

parser.addRule(MATCH_EMPTY_LI, ""); // remove empty li elements

parser.addRule(MATCH_BLANK_P_TAG, ""); // remove empty lines
...
const fixImagesInHtml = (html: string) =>
  html.replace(
    /<img([^>]+)\ssrc=(['"])([^'"]+)\2/gi,
    `<img$1 src=$2${"http://localhost:5244/api"}/$3$2`
  );
