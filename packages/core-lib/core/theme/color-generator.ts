export class ColorGenerator {
    ...
    lightenColor(darknessPercentage: number, transparencyPercentage = 0) {
        return this.blendColors(this.white, darknessPercentage * 0.01, transparencyPercentage * 0.01);
    }

    darkenColor(darknessPercentage: number, transparencyPercentage = 0) {
        return this.blendColors(this.black, darknessPercentage * 0.01, transparencyPercentage * 0.01);
    }

    private blendColors(secondColor: string, secondColorAlpha: number, opacity: number) {
        const rgbColor1 = this.hex2rgb(this.color.includes('rgb') ? this.rgb2hex(this.color) : this.color)
            .match(/\d+/g)
            ?.map(Number) ?? [];

        const rgbColor2 = this.hex2rgb(secondColor.includes('rgb') ? this.rgb2hex(secondColor) : secondColor)
            .match(/\d+/g)
            ?.map(Number) ?? [];
        ...
        return '#' + rgbBlendedColor.map(this.intToHex).join('') + (opacity ? Math.floor(opacity * 255).toString(16) : '');
    }
